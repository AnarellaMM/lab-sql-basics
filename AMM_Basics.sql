...
ORDER BY date DESC;
 ...
 -- QUERY 17 in the loan table, for each day in December 1997, count the number of loans issued for each unique 
 -- loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your output.
...
FROM (
    SELECT account_id,
        SUM(CASE WHEN type = 'PRIJEM' THEN amount ELSE -amount END) AS difference,
        RANK() OVER (ORDER BY difference DESC) AS rank
    FROM trans
    GROUP BY account_id
) AS ranked_accounts
WHERE rank <= 10;

-- Hoow do I solve this?
